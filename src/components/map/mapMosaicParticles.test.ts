...
describe('createMapMosaicParticles', () => {
  it('adds a top-only additive overlay that reuses the source geometry', () => {
    ...
    expect(topMaterial.fragmentShader).toContain('sampleGrid(float cellWorld, out float gapCoverage)')
    expect(topMaterial.fragmentShader).toContain('gapCoverage = 1.0 - square;')
    expect(topMaterial.fragmentShader).not.toContain('gapCoverage = (1.0 - square) * activity;')
